          ...
          const task = (async () => {
            ...
            setStore(
              produce((draft) => {
                ...
                const hydratedFetched = fetched.map((message) => {
                ...
                for (const message of currentMessages) {
                  if (!tracker.messages.has(message.id)) continue
                  if (fetchedIDs.has(message.id)) continue
                  hydratedFetched.push({
                    info: message,
                    parts: draft.part[message.id] ?? [],
                  })
                }
                hydratedFetched.sort((a, b) => a.info.id.localeCompare(b.info.id))
                const merged = mergeFetchedMessages({
                  currentMessages,
                  currentParts: draft.part,
                  fetched: hydratedFetched,
                  optimisticMessages,
                })
